...
### 不正アクセス対策
Filterを利用し、未ログイン状態で管理画面URLへ直接アクセスした場合はログイン画面へリダイレクトする処理を実装しました。
また、ログイン済みユーザーがログイン画面へアクセスした場合はホーム画面へリダイレクトすることで、ユーザー体験の向上を意識しました。

### 一覧表示の工夫
会員一覧画面では、データ件数が増えた場合でも見やすく操作しやすいよう、
ページネーションを実装しました。
MyBatisでlimit / offsetを用いて1ページあたりの表示件数を制御しています。
